...
void hotkeysGetCommand(client *c) {
    ...
    if (c->argc > 2) {
        if (c->argc == 4 && !strcasecmp(objectGetVal(c->argv[2]), "TYPE")) {
            char *type_str = objectGetVal(c->argv[3]);
...
size_t hotkeyCMSUpdate(hotkeyCMS *hotkey_cms, robj *key) {
    if (!hotkey_cms || !key || !objectGetVal(key)) {
        return 0;
    }
...
static hotkeyStatEntry *getOrCreateHotkeyStatEntry(dict *hotkeys, robj *key) {
    if (!hotkeys || !key || !objectGetVal(key)) {
        return NULL;
    }

    dictEntry *de = dictFind(hotkeys, objectGetVal(key));
    if (de) {
    ...
    sds key_sds = sdsnew(objectGetVal(key));
    if (!key_sds) {
        zfree(entry);
        return NULL;
    }
