start_server {tags {"hotkey"}} {
    ...
    test "Hotkey CMS bucket size configuration" {
        r config set hotkey-enabled yes
        # The stored config value round-trips; the CMS rounds its internal
        # width up to the nearest power of two internally.
        r config set hotkey-cms-bucket-size 4096
        assert_equal [lindex [r config get hotkey-cms-bucket-size] 1] "4096"
        r config set hotkey-cms-bucket-size 8192
        assert_equal [lindex [r config get hotkey-cms-bucket-size] 1] "8192"
        r config set hotkey-cms-depth 4
        assert_equal [lindex [r config get hotkey-cms-depth] 1] "4"
    }
    ...
    test "Test hotkey runtime metrics" {
        ...
        assert_match "*hotkey_cms_bucket_size:*" $info_after
    }
    ...
    test "QPS reported as a positive decayed rate" {
        ...
        # depends on real elapsed time, so we assert it is a sane positive
        # value rather than an exact deterministic figure.
    ...
    test "Cold keys fade out under decay" {
        ...
        # under the 0.5 eviction floor after ~7s and the key is dropped on the
        # next query's lazy decay.
        after 7000
