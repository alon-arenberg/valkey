start_server {tags {"hotkey"}} {
    ...
    test "Hotkey CMS bucket size auto-adjustment" {
        ...
        assert_equal [lindex [r config get hotkey-cms-bucket-size] 1] "1024"
    }
    ...
    test "Top-k tracks the hottest keys" {
        r config set hotkey-enabled yes
        r hotkeys reset
        r config set hotkey-sampling-ratio 100
        ...
        set hotkeys [r hotkeys get]
        puts "Top-k test: [llength $hotkeys] hotkeys detected"
        # With top-k=3, we should see at most 3 keys per type
        assert {[llength $hotkeys] > 0}
